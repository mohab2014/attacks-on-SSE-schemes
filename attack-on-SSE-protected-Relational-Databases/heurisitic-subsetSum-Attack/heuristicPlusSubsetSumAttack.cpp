...
//g++ -o heuristicAttack heuristicPlusSubsetSumAttack.cpp -O3 -std=c++11
#include <cstdlib>
   ...
   else
   {
     ...
     do
     {
         ...
         if(recoveredQueries.size() > 0)
         {
             ...
             sort(recoveredQueries.begin(),recoveredQueries.end(),CompQueryNo());
             cout<<"SubsetSUM Section: recovered Queries size = "<<recoveredQueries.size()<<endl;
             //cout<<toString(recoveredQueries)<<endl;
             ...
             std::sort (queries.begin(), queries.end(),CompQueryListSize());
             recoveredQueries.clear();    
             
         }
         
         
         
         cout<<"Subset SUM LOOP NO = "<<subset_sum_loop<<endl;
         
         for(int i = 1;i <= maxNoQueries;i++)
         {
             int size_i = queries[i].queryList.size();
             
             if(size_i != 0 && size_i <= 60)
             {
                  ...
                  //try to solve the subset sum problem
                  
                  vector<Query> currentQueryList = queries[i].queryList;
                  Query head = currentQueryList[0];
                  int target_sum = number_of_records-head.result_size;
                  
                  
                  cout<<"size of "<<i<<"th list = "<<size_i<<", main head = "<<queries[i].queryList[0]<<", target sum = "<<target_sum<<endl;
                  
                  for(int j = 0;j < size_i;j++)
                      cout<<queries[i].queryList[j].query_no<<":"<<queries[i].queryList[j].result_size<<";";
                  cout<<endl;
          
                  
                  currentQueryList.erase(currentQueryList.begin());
                  /*cout<<"without head:"<<endl;
                  for(int j = 0;j < size_i;j++)
                      cout<<currentQueryList[j].query_no<<":"<<currentQueryList[j].result_size<<";";
                  cout<<endl;
                  */
                  if(subsetSum(currentQueryList , target_sum ))
                  {
                     ...
                     p.push_back(head); //let each solution include the first element in queryList since sum = number_of_records-queryList[0].resultsize
             
                     output(currentQueryList, currentQueryList.size() - 1, target_sum, p, targetAttributes);
                     
                     int no_solutions = correctSolutions.size();
                     
                     cout<<"current no of solutions = "<<no_solutions<<endl;
                     
                     if(no_solutions == 1)
                     {
                         vector<Query> sol = correctSolutions[0];
          
                         cout<<"One solution exists at i = "<<i<<" of size = "<<sol.size()<<endl; 
                        ...
                        for(int i = 0;i < sol.size();i++)
                         ...
                         display(sol);
                         outMetaFile<<"Subset sum solution of size = "<<sol.size()<<": "<<toString(sol)<<endl;
                     ...
                     else if(no_solutions)
                     {
                             ...
                             cout<<no_solutions<<" possible solutions exist at i = "<<i<<". So we compute their intersections.\n";
                          ...
                          /*if(no_solutions == 2)
                          {
                            for(int i = 0;i < correctSolutions.size();i++)
                            {
                              ...
                              vector<Query> sol = correctSolutions[i];
          
                              cout<<"Solution exists at i = "<<i<<" of size = "<<sol.size()<<", head = "<<sol[0]<<endl; 
                            ...
                            if(correctSolutions[0].size() <= correctSolutions[1].size())
                            {
                                sol = efficientIntersection(correctSolutions[0],correctSolutions[1]);
                           
                            }
                            else
                                sol = efficientIntersection(correctSolutions[1],correctSolutions[0]);
                                
                                
                                
                            outMetaFile<<"Incomplete subset sum solution of size = "<<sol.size()<<": "<<toString(sol)<<endl;
                                
                            for(int i = 0;i < sol.size();i++)
                                      recoveredQueries.push_back(sol[i]);
                                
                                
                            IncompleteSolution inc;
                                
                            if(correctSolutions[0].size() == correctSolutions[1].size())
                                inc.card = correctSolutions[0].size();
                            else
                                inc.card = -1;
                            
                            
                            outMetaFile<<"Incomplete subset sum solution of size = "<<sol.size()<<", card = "<<inc.card<<": "<<toString(sol)<<endl;
                            inc.intersection = sol;
                            inc.solutions[0] = correctSolutions[0];
                            inc.solutions[1] = correctSolutions[1];
                                
                            incompleteSols.push_back(inc);
                            correctSolutions.clear();
                            break;
                         ...
                         vector<Query> result;
                         int temp;
                         if(correctSolutions.size())
                         {
                             result = correctSolutions[0];
                             temp = result.size();
                             sort(result.begin(),result.end(),CompQueryNo()); //sorting according to query no since query result size is repetitive
           
                         
                            for(int s = 1;s < correctSolutions.size();s++)
                            {
                             //sort(corectSolutions2.begin(),corectSolutions2.end(),CompQueryNo()); //sorting according to query no since query result size is repetitive
                             if(correctSolutions[s].size() != temp)
                             {
                                 //cout<<"Discrepancy "<<temp<<", "<<correctSolutions[s].size()<<endl;
                                 //outMetaFile<<"Discrepancy "<<temp<<", "<<correctSolutions[s].size()<<endl;
                                 result.clear();
                                 break;
                            
                             }
                             result = efficientIntersection(result,correctSolutions[s]);//intersection(intersectionSet[i],jQueryList);
                            ...
                            if(result.size())
                            {
                              //cout<<"result of intersection size = "<<result.size()<<endl;
                              sort(result.begin(),result.end(),CompQueryNo());
                              //cout<<toString(result)<<endl;
                            
                            
                              //cout<<"Incomplete subset sum solution of size = "<<result.size()<<", poss card = "<<temp<<": "<<toString(result)<<endl;
                            
                              //outMetaFile<<"Incomplete subset sum solution of size = "<<result.size()<<", poss card = "<<temp<<": "<<toString(result)<<endl;
                            
                              for(int i = 0;i < result.size();i++)
                                      recoveredQueries.push_back(result[i]);
                            }
                            else
                            {
                                cout<<"No valid intersection since sets have different sizes"<<endl;
                            }
                         }
                         correctSolutions.clear();
                     ...
                     {
                         cout<<"Non valid solutions could be found. # of valid solutions = 0"<<endl;
                     }
    
              
                  }
                  else
                      cout<<"no solutions found with sum equal to the number of records."<<endl;
    ...
    }while(recoveredQueries.size() > 0);
   
    if(subset_sum_loop > 1)
        decreasing = true;
   }
